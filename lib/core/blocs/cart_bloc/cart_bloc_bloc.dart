...
class CartBloc extends Bloc<CartBlocEvent, CartBlocState> {
  ...
  if (state is CartBlocLoaded) {
    ...
    Map<String, List<Map<ProductModel, int>>> foodCart = Map.from((state as CartBlocLoaded).foodCart);
    Map<String, List<Map<ProductModel, int>>> productCart = Map.from((state as CartBlocLoaded).productCart);

    bool isFood = event.product.isFood;
    Map<String, List<Map<ProductModel, int>>> targetCart = isFood ? foodCart : productCart;

    if (targetCart.containsKey(event.product.category)) {
      List<Map<ProductModel, int>> categoryList = List.from(targetCart[event.product.category]!);
